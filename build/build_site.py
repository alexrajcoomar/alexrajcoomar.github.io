...
SW_TEMPLATE = r"""/* Offline machinery for the whole site. Generated by
   ...
   and its versioned name retires it whenever a cached file changes. SAVED
...
// The two digests stay plain string literals on their own lines. The
// register digests this worker's behaviour with those two lines blanked,
// so that a publish which only changes what is cached does not read as a
...
async function migrate() {
  ...
  const old = keys.filter(k => k !== SAVED
    && (k.indexOf(NS + "pages-") === 0 || (LEGACY(k) && k.indexOf("site-pages-") === 0)));
  if (!old.length) return false;
  const nc = await caches.open(SAVED);
  for (const k of old) {
...
self.addEventListener("activate", e => {
  e.waitUntil((async () => {
    ...
    await Promise.all(keys
      .filter(k => k !== CORE && k !== SAVED && (MINE(k) || LEGACY(k)))
      ...
      const refresh = fetch(req)
        .then(res => {
          if (res && res.ok && res.type === "basic") {
            ...
            caches.open(SAVED).then(c => c.put(req, copy)).catch(() => {});
          }
          return res;
        });
...
self.addEventListener("message", e => {
  ...
  if (msg.type === "cache-all") {
    ...
    e.waitUntil(caches.delete(SAVED).then(() => tell({ type: "drop-all-done" })));
  }
});
"""
